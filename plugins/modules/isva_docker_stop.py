...
DOCUMENTATION = r'''
---
module: isva_docker_stop
short_description: Stop the configuration container
description:
...
EXAMPLES = r'''
- name: Stop the configuration container
  isva_docker_stop:
    state: stopped
'''
...
from ansible_collections.community.isva.plugins.module_utils.isva_docker_stop import stop_container
...
class ArgumentSpec(object):
  def __init__(self):
    ...
    argument_spec = dict(
      state=dict(type='str', required=True, choices=['stopped']),
      log_level=dict(type='str', default='INFO', choices=['CRITICAL', 'FATAL', 'ERROR', 'WARN', 'WARNING', 'INFO', 'DEBUG', 'NOTSET'])
    )
    self.argument_spec = {}
    self.argument_spec.update(argument_spec)

def __exec_stopped(module):
  check_mode = module.check_mode
  logger.debug('Stopping the docker configuration container')
  if check_mode:
    return {'changed': True, 'after': {'state': 'stopped'}}
...
def exec_module(module):
  ...
  if state == 'stopped':
    response = __exec_stopped(module=module)
    return {'changed': response['changed'], 'diff': {'before': {'state': 'started'}, 'after': response['after']}}
