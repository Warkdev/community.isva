...
class ArgumentSpec(object):
  def __init__(self):
    ...
    argument_spec = dict(
      state=dict(type='str', required=True, choices=['gathered', 'deploy', 'rollback']),
...
def __exec_deploy(module, **kwargs):
...
def __exec_rollback(module, **kwargs):
  check_mode = module.check_mode
  before = kwargs
...
def exec_module(module):
  ...
  if state == 'gathered':
    ...
    response = __exec_deploy(module=module, **before)
    return {'changed': response['changed'], 'diff': {'before': before, 'after': response['after']}}
  elif state == 'rollback':
    before = __exec_gathered(module=module)
    response = __exec_rollback(module=module, **before)
    return {'changed': response['changed'], 'diff': {'before': before, 'after': response['after']}}
